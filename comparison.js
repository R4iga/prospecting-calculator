...
function renderComparison() {
  ...
  var html = '<table style="width:100%;border-collapse:collapse;font-size:0.85rem;line-height:1.8;">';
  
  // Header
  html += '<tr style="background:var(--bg-card);">';
  html += '<th style="padding:8px;">Stat</th>';
  compareItems.forEach(function(item) {
    html += '<th style="padding:8px;text-align:center;color:var(--cyan);">' + item.name + '</th>';
  });
  html += '</tr>';

  // Rarity
  html += '<tr>';
  html += '<td style="padding:6px;font-weight:600;">Rarity</td>';
  ...
  compareItems.forEach(function(item, idx) {
    ...
    html += '<td style="padding:6px;text-align:center;' + fw + '"><span style="color:' + color + ';">' + (item.rarity || '—') + '</span></td>';
  });
  html += '</tr>';
  ...
  Object.keys(allKeys).sort().forEach(function(key) {
    html += '<tr>';
    html += '<td style="padding:6px;font-weight:600;">' + key.replace(/_/g, ' ') + '</td>';
    var values = [];
    compareItems.forEach(function(item) {
    ...
    values.forEach(function(v, idx) {
      if (v === undefined) { html += '<td style="padding:6px;text-align:center;color:var(--text-dim);">—</td>'; return; }
      ...
      if (idx === bestIdx) {
        ...
        if (diffs.length) diff = ' <span style="color:var(--yellow);font-size:0.75rem;">(' + diffs.join(', ') + ')</span>';
      }
      html += '<td style="padding:6px;text-align:center;color:' + color + ';' + fw + '">' + v + diff + '</td>';
    });
    html += '</tr>';
  });
  ...
  // Materials
  html += '<tr>';
  html += '<td style="padding:6px;font-weight:600;">Materials</td>';
  compareItems.forEach(function(item) {
    ...
    html += '<td style="padding:6px;text-align:center;">' + count + ' items</td>';
  });
  html += '</tr>';

  // Cost
  html += '<tr>';
  html += '<td style="padding:6px;font-weight:600;">Cost</td>';
  compareItems.forEach(function(item) {
    ...
    html += '<td style="padding:6px;text-align:center;">' + display + '</td>';
  });
  html += '</tr>';
